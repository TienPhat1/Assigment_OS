...
int translate(address_t virtual, address_t * physical) {
	...
	// to physical address.
	index_t getindex = get_index(virtual);
	offset_t getoffset = get_offset(virtual);
	int i = 0, j = 0 ;
	getoffset = getoffset | (0xffffffff << OFF_LEN);
	//check if the getindex is inside the tlb
	while(i < TLB_SIZE-1 && getindex != tlb[i].virtual) i++;
	if(i != TLB_SIZE -1){
		//TODO:translate index from virtual to physical by tlb
		*physical = tlb[i].physical;
		*physical = (*physical<<OFF_LEN) | (0xffffffff >> (32 - OFF_LEN));
		*physical = *physical & getoffset;
		tlb[i].count++;
		return 1;
	}
	else{
		while(j < (PAGE_TABLE_SIZE-1) && getindex != page_table[j].virtual) j++;
		if(j < PAGE_TABLE_SIZE - 1)
		{
			//TODO: translate index from virtual to physical by page_table
			*physical = page_table[j].physical;
			*physical = (*physical << OFF_LEN) | (0xffffffff >> (32 - OFF_LEN));
			*physical = *physical & getoffset;
			page_table[j].count++;
			return 1;
